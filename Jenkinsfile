...
//  This stage only run on first time
                    ...
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                          dir('kubernetes/') {
                             sh '''
                                 ...
                                 curl -u admin:$docker_password http://104.196.124.19:8081/repository/helm-hosted/ --upload-file myapp-${helmversion}.tgz -v
                            '''
                          }
                    }
                }
 ...
 //       }
        ...
        stage('Deploying application on k8s cluster') {
            steps {
               script{
                   withCredentials([sshUserPrivateKey(credentialsId: 'k8smaster-connect', keyFileVariable: 'k8s_connect')]) {
                        dir('kubernetes/') {
                          sh 'ssh -i "$k8s_connect" 35.231.7.26 -l jenkins "helm upgrade --install --set image.repository="104.196.124.19:8083/springapp" --set image.tag="${VERSION}" myjavaapp myapp/ "' 
                        }
                    }
               }
            }
